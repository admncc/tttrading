<!-- Reproduce: scratchpad/fetch_all.py -> bundle.json, then (from apps/server):
     node --import tsx src/scripts/backtestRules.ts <so.json> <bundle.json> ../../docs/phase1-rule-backtest.md
     Numbers computed by the script; do not edit by hand. -->
...
| neutral | 6 | 6 | 50.0% (n=12, CI 25.4%–74.6%) | +0.88R ±0.78 (n=14) | +0.85R (n=13) |
| negative | 11 | 11 | 50.0% (n=22, CI 30.7%–69.3%) | +0.02R ±0.22 (n=24) | -0.04R (n=22) |
...
| >3.5 | 16 | 56.3% (n=16, CI 33.2%–76.9%) | +0.32R ±0.35 |
...
- Climatology baseline (always predict the 42.1% base rate): **0.244** — the reference for the skill score (T3).
- Geometry baseline (pBase = slDist/(slDist+tpDist)): **0.271** · skill vs clim -0.112
- Old SO (score/100, uncalibrated): 0.275 · skill -0.128
- New SO (score/100, uncalibrated): 0.288 · skill -0.180

> Brier skill score = 1 − Brier/climatology; > 0 beats "always predict the base rate", ≤ 0 doesn't. On this tiny, one-regime sample it's a reference point, not a verdict.
...
unresolved: 18 · win: 18 · noData: 11 · loss: 22 · notFilled: 8 · timeout: 4

Unresolved age (n=18): <3d 8 · 3–7d 6 · 7–14d 4 · >14d 0. (>14d unresolved would be a bug — the horizon is 14d.)

## Phantom wins + reproducibility (P1-R8)
- Phantom WINS (old engine called TP-first, repaired engine says never filled): SPX, SPX, PUMP, BTC, GRASS, ZRO — matches the Phase-0 relabel.
- All never-filled/ambiguous this run (incl. old non-wins): SPX, SPX, PUMP, BTC, GRASS, ZRO, BNB, INJ.
